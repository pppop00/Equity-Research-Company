...
- **QC / deliberation transparency (encouraged):** When dual-QC resolution **materially changes** a score or narrative versus the first draft, add **one or two short sentences** (analyst tone) grounded in `qc_audit_trail.json` and finalized `porter_analysis.json`. The **full** audit trail remains in **`qc_audit_trail.json`** and appendix / `qc_deliberation` summaries.
- **When QC changes a radar score** (vs the first draft), use this **recommended skeleton** inside the matching `<li>` (wording may vary; keep all four elements):  
  **"Dual-QC deliberation held that …, and adjusted the [force] score from *a* to *b*, because …; …"** (then continue with the usual evidence and analysis). Use integers **1–5** for *a* and *b*. Do **not** use a bullet that is only an internal ticket stub with no substance; optional QC item id in parentheses at the end of a sentence. If a force’s score **did not** change, you do **not** need this sentence — write normal analysis.
- **Source text:** `porter_analysis.json` fields under `company_perspective`, `industry_perspective`, and `forward_perspective` should store lines that map cleanly into the five `<li>` elements; if scores moved after QC, the same **from–to + reason** content should appear in JSON and HTML.
