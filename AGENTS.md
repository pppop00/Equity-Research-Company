...
2. **`agents/`** — Sub-task instruction files. The orchestrator runs Phase 1 **Agents 1–3** in parallel (financials, macro, news), then **Agent 4** (`edge_insight_writer.md`) writes **`edge_insights.json`** before Phase 2; optional **dual QC** on macro/Porter then **`qc_resolution_merge.md`**; Phase 5 uses **`report_writer_*.md`**; **`final_report_data_validator.md`** performs the final professional data validation pass; **`report_validator.md`** then performs the final HTML structure / delivery validation. Workspace holds JSON throughout (plus HTML after Phase 5).
